            ...
            function role_str(){
                ...
                if($("#role-select .select-box li ").find("input[type='radio']:checked").length>0){
                    ...
                    $("#role-select .select-box li input[type='radio']:checked").each(function () {
                        i++;
                        st_id=$(this).parent().parent().attr('data-st_id');
                        prv=$(this).val();
            ...
            $('#revise').on('click','#roleRevise-submit', function () {
                ...
                if(RegEx_judgment==true){
                    $.ajax({
                        ...
                        data: {sessionId:sessionStorage['sessionid'],jdata: $.trim(cu_str),cu_id:$.trim(cu_id), cu_code: $.trim(cu_code),cu_name:$.trim(cu_name),role_id:$.trim(role_id),login_name:$.trim(login_name)},
                        beforeSend: function () {
...
$('#cu_add_submit').click(function () {
    ...
    if(RegEx_judgment==true){
            $.ajax({
                ...
                data: {sessionId:sessionStorage['sessionid'],jdata: $.trim(cu_str), cu_code: $.trim(cu_code),cu_name:$.trim(cu_name),role_id:$.trim(role_id),login_name:$.trim(login_name),login_pwd:$.trim(login_pwd)},
                beforeSend: function () {
                    $('.loading').addClass('loadingin');
                }
                ,error: function () {
...
//单击选中，双击取消选中
$('#role-select').on('dblclick','.multiselect lable', function () {
    $(this).find("input[type='radio']").prop('checked',false);
    var st_val1=$(this).find("input[type='radio']").val();
    $(this).parent().parent().parent().find(`[value=${st_val1}]`).each(function () {
        $(this).prop('checked',false);
    });
});
$('#role-select').on('click','.multiselect lable', function () {
    var st_val2=$(this).find("input[type='radio']").val();
    $(this).parent().parent().parent().find(`[value=${st_val2}]`).each(function () {
        $(this).prop('checked',true);
    });
});
$("#role-select").on('click',' li lable',function () {
    $(this).parent().find("input[type='radio']").each(function () {
        $(this).prop('checked',false);
    });
    $(this).find('input[type="radio"]').prop('checked',true);
});
$("#role-select").on('dblclick',' li lable',function () {
    $(this).find('input[type="radio"]').prop('checked',false);
});
