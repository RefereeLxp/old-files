...
function isPredefinedModel(modelStorage) {
    var result = false;
    var types = ["door","window","cable"];
    var model = JSON.parse(modelStorage);
    if (model.type && types.indexOf(model.type) > -1) {
        result = model.type
    }
    return result;
}
