...
$('#example2 tbody').on('click','.role-revise', function () {
    var role_id=$(this).parent().parent().find('td').eq(0).html();
    var gt_id='';
    $("#role-select input").each(function () {
        $(this).attr("checked",false);
    });
    $.ajax({
        type: 'GET',
        data:{sessionId:sessionStorage['sessionid'],role_id:role_id},
        url: url_top+'monitor/prv/role/getOwnGrant',
        success: function(list){
            var html = '';
            var gt_id=[];
            var data=[];
            $.each(list.roleGrant, function(i, p){
                data[i]=p.gt_id;
               $("#role-select input[value = "+ p.gt_id +"]").prop("checked",true);
            });
        }
    });
    //修改角色权限确认按钮
