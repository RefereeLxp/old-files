...
if(sessionStorage['host_id']!=null){
    $.ajax({
        type: 'GET',
        data: {sessionId: sessionStorage['sessionid']},
        url: 'http://202.119.65.27:9999/monitor/ass/controller/getAll',
        success: function (list) {
            var html2 = '';
            if (list.length == 0) {
                console.log('错误');
...
$('#insH').click(function(){
    ...
    if(RegEx_judgment==true){
        if($.inArray('', arr)==-1){
            $.ajax({
                url:'http://202.119.65.27:9999/monitor/ass/host/create',
                data:{sessionId:sessionStorage['sessionid'],host_code:encodeURI(host_code),host_name:encodeURI(host_name,'UTF-8'),host_key: encodeURI(host_key,'UTF-8'),st_id:st_id,ip_address:ip_address},
                success:function(){
                    $.ajax({
                        url:'http://202.119.65.27:9999/monitor/ass/dfloor/create',
...
$('#revise').on('click','#hostchange',function () {
    ...
    if(RegEx_judgment==true){
        if($.inArray('', arr)==-1){
            ...
            $.ajax({
                type: 'POST',
                url: 'http://202.119.65.27:9999/monitor/ass/host/update',
                data: {sessionId:sessionStorage['sessionid'],host_id:host_id,host_code: host_code, host_name:host_name,host_key:host_key,st_id:st_id,ip_address:ip_address},
                success: function(){
                    alert('已修改');
                    location.reload();
                    $("#example2").DataTable();
...
$('#ctAdd').click(function () {
    ...
    $('#ctAdd-submit').click(function () {
        var ctrl_code=$('#maintenance1 form').find('input').eq(0).val(),
            ctrl_name=$('#maintenance1 form').find('input').eq(1).val(),
            host_id=$('#maintenance1').find("[data-target='#host-select']").attr('data-host_id'),
            room_id=$('#room-select').val(),
            ip_address=$('#maintenance1 form').find('input').eq(3).val(),
            mac=$('#maintenance1 form').find('input').eq(3).val(),
            class_id=$('#dict-select option:selected').val(),
            threshold_h=$('#maintenance1 form').find('input').eq(5).val(),
            threshold_l=$('#maintenance1 form').find('input').eq(6).val();
        ...
        if(RegEx_judgment==true){
            if($.inArray('', arr)==-1){
                $.ajax({
                    type: 'POST',
                    url: ' http://202.119.65.27:9999/monitor/ass/controller/create',
                    data: {sessionId:sessionStorage['sessionid'],ctrl_code:encodeURI(ctrl_code,'UTF-8'), ctrl_name: encodeURI(ctrl_name,'UTF-8'),host_id:host_id,room_id: room_id, ip_address:encodeURI(ip_address,'UTF-8'),mac:mac,dict_id:1, threshold_h: threshold_h,threshold_l:threshold_l},
                    success: function(){
                        alert('已保存成功');
                        location.reload();
                        $("#example3").DataTable();
...
$('#example3').on('click','.ct_revise',function () {
    if(($('#binding2').css('display') == 'none')){
        $('#binding2').find('.main-title').empty().html('添加项目');
        $('#binding2').find(".head-box input[type='text']").each(function () {
            $(this).parent().next().empty();
        });
        $('#binding2').find(".head-box select").each(function () {
            $(this).parent().next().empty();
        });
        $('#binding2').find(".head-box input[type='button']").each(function () {
            $(this).parent().next().empty();
        });
    }
    ...
    $('#ctRevise-submit').click(function () {
        var ctrl_code=$('#revise form').find('input').eq(0).val(),
            ctrl_name=$('#revise form').find('input').eq(1).val(),
            room_id=$('#room-select').val(),
            ip_address=$('#revise form').find('input').eq(3).val(),
            mac=$('#revise form').find('input').eq(4).val(),
            class_id=$('#dict-select').val(),
            threshold_h=$('#revise form').find('input').eq(5).val(),
            threshold_l=$('#revise form').find('input').eq(6).val();
        host_id=$('#revise').find("[data-target='#host-select']").attr('data-host_id');
        console.log(ctrl_code);
        ...
        if(RegEx_judgment==true){
            if($.inArray('', arr)==-1){
                $.ajax({
                    type: 'POST',
                    url: ' http://202.119.65.27:9999/monitor/ass/controller/update',
                    data: {sessionId:sessionStorage['sessionid'],ctrl_id:ctrl_id,ctrl_code:encodeURI(ctrl_code,'UTF-8'), ctrl_name: encodeURI(ctrl_name,'UTF-8'),host_id:host_id,room_id: room_id, ip_address:encodeURI(ip_address,'UTF-8'),mac:mac,dict_id:class_id, threshold_h: threshold_h,threshold_l:threshold_l},
                    success: function(){
                        location.reload();
                        $("#example3").DataTable();
                        clear_host_select();
...
$('#revise').on('click','#bindR',function(){
    var data=$('#binding2');
    add_data_type(data);
    var value=$('#correct .col-xs-12:eq(0) input').val();
    $('#newBind2 input[name="ctrl_code"]').val(value);
    $.ajax({
        url:'http://202.119.65.27:9999/monitor/ass/device/findByCcode',
...
$(function(){
        ...
        $('#bindGo2').click(function(){
            if($('#newBind2 input[name="id"]').val()!=-1) {
                ...
                if(RegEx_judgment==true){
                    $.ajax({
                        url: 'http://202.119.65.27:9999/monitor/ass/device/update',
                        data: data,
                ...
                if(RegEx_judgment==true){
                    $.ajax({
                        url:'http://202.119.65.27:9999/monitor/ass/device/create',
                        data:data,
                        headers:{"Content-Type":"application/x-www-form-urlencoded;charset=UTF-8"},
                        success:function(){
                            alert('成功')
...
$('#example3').on('click','a[data-oper="delete"]',function(e){
    ...
    console.error(did);
    $.ajax({
        url:'http://202.119.65.27:9999/monitor/ass/controller/destroy',
        data:{sessionId:sessionStorage['sessionid'],ctrl_id: did},
        success:function(){
            $.ajax({
                url:'http://202.119.65.27:9999/monitor/ass/device/delete',
...
$('#example2').on('click','a[data-oper="delete"]',function(e){
    ...
    $.ajax({
        url:'http://202.119.65.27:9999/monitor/ass/host/destroy',
        data:{sessionId:sessionStorage['sessionid'],host_id: hid},
        success:function(){
            $.ajax({
                url:'http://202.119.65.27:9999/monitor/ass/dfloor/delete',
                data:{host_id:hid},
                success:function(){
