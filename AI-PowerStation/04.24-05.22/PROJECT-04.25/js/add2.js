...
function show1(oper) {
  ...
  <div class="col-xs-12"><span>${title1[j].innerHTML}</span><input type="text" value="${content[j].innerHTML}"/></div>`
            ...
            html1 += `
  <div class="col-xs-12"><span>${title1[j].innerHTML}</span><span><select style="width:200px">
                                <option>南京</option>
                                <option>上海</option>
                                <option>广州</option>
                            </select></span></div>`
    ...
    html1 += `</div><div class="row"><div class="col-xs-12"></div><div class="col-xs-4"></div><div class="col-xs-4"></div><div class="col-xs-4"><botton type="button" class="btn my-btn" id="stationChange1">保存</botton></div></div>`;
    $('#correct').html(html1)
}