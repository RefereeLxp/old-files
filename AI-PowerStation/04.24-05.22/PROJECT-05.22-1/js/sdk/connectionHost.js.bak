  ...
  if (tempJson.status.statusStep == "synch" && tempJson.status.statusValue == 1) {
            ...
            (function(k) {
                if (temp[k].status != null && temp[k].status.hasOwnProperty('alarm')) {
                    if (temp[k].status.alarm == 1) {
                        $.ajax({
                            url: 'http://202.119.65.27:9999/monitor/ass/device/findIdByCcode',
                            ...
                            success: function (text) {
                                ...
                                if (temp[k].deviceType == 'ZIGBEE_DipSensor') {
                        ...
                        $.ajax({
                            url: 'http://202.119.65.27:9999/monitor/mon/alert/saveAlert',
                            data: {
    ...
    var timer=setInterval(
        function(){
            ...
            if(lastkey!=null) {
                $('#jiade').html(jinggao[lastkey]);
                $('#jiade').click(function(){
                    locateDevice(lastkey);
                })
                var html=[]
                ...
                if(html!=[]) {
                    ...
                    $('#alertM').html(b);
                    $('.label-warning').html(jinggao.length);
                }
  ...
  var tempJson = JSON.parse(temp);
    if (tempJson.status!=null&&tempJson.status.hasOwnProperty('alarm')) {
        ...
        if (tempJson.status.type == "℃" && (tempJson.status.value >= 20 || tempJson.status.value <= 0)) {
            ...
            $.ajax({
                url: 'http://202.119.65.27:9999/monitor/mon/history/saveHistory',
