...
V3D.FloorView = function (camera, scene, renderer, controls) {
    ...
    var dragObjects = [];
    var placeNodes = [];
    var wallDepthReal =  V3DSettings.WALL_DEPTH;
    var wallHeightReal = V3DSettings.WALL_HEIGHT;
    var walls = [];
    var delteDevice;
    ...
    scope.showHideInfrared = function (deviceId, flag) {
    ...
    scope.showHidePerson = function (deviceId, flag) {
        var pnode = getPnodeByDeviceId(deviceId);
        if (pnode) {
            if (flag)
                pnode.showDetectedPerson();
            else
                pnode.hideDetectedPerson();
        }
    };


    scope.showHideLight = function (deviceId, flag) {
        var pnode = getPnodeByDeviceId(deviceId);
        if (pnode) {
            if (flag)
                pnode.showLight();
    ...
    function initLoadingModels(models) {
        ...
        $.each(models, function (idx, data) {
            if (!CACHEDATA["Model:" + data.id]) {
                if (!isPredefinedModel(data.storage)) {
    ...
    var heatmap = null;
    scope.showHeatMap = function (isShow,dataArray) {
        if (isShow) {
            var areaPts = [];
            var shape =null;
            $.each(scope.floorShapedata.areas,function (idx,area) {
                if(area.isFloor == true)
                    shape = area.points;
            });
            ...
            if(shape)
            {
                ...
                $.each(dataArray,function (idx,value) {
                    var pnode = getPnodeByDeviceId(value.deviceid);
                    if(pnode){
                        console.log('pnode');
                        console.log(pnode);
                        var p3d = pnode.visual3d.position;
                        var p2d = new THREE.Vector2(p3d.x, p3d.z);
                        var p = new THREE.Vector2().subVectors(p2d, floorsize.min).multiplyScalar(20);
                        var dataPoint = {
                            x: Math.floor(p.x),
                ...
                if(points&&points.length>0){
                    var mesh = heatmap.getMesh(geomotry, getFloorSize2D(), points);
                    mesh.position.y = 1 * V3DSettings.RESIZE_RATIO;
                    visualRoot.add(mesh);
                }
            }
        }
    ...
    function showShape(areas) {
        ...
        if (areas) {
            $.each(areas, function (idx, area) {
                ...
                {
                    var areaMesh = new THREE.Mesh(floorShpeGeomotry, material);
                    areaMesh.rotateX(Math.PI / 2);
                    floorGroup.add(areaMesh);
                }
    ...
    function autoPosition(pnode, wall) {
        ...
        if (wall) {
            ...
            var wallFrom = wall.userData.from;
            var wallTo = wall.userData.to;
            ...
            pnode.updatePosition(location);
        }
    }


    function makeHole(object,size) {
        var wall = null;
        ...
        if (wall) {
            ...
            hole.rotation.set(wall.rotation.x, wall.rotation.y, wall.rotation.z);
            var hole_bsp = new ThreeBSP(hole);

            var transparent = wall.userData.data.opacity !=1 ;
            var opacity = transparent?wall.userData.data.opacity :1;
            ...
            //autoPosition(pnode, wall)
        }
    }


    function showWallsegment(from, to, isFromAreaShape,data) {
        ...
        var transparent = data.opacity !=1 ;
        var opacity = transparent?data.opacity :1;
    ...
    function showWall(walls) {
        if (walls) {
            $.each(walls, function (idx, wall) {
                for (var i = 0; i < wall.points.length - 1; i++) {
                    var from = wall.points[i],
                        to = wall.points[i + 1];
                    var hasjoint = i < wall.points.length - 2 || wall.fromArea;
                    showWallsegment(from, to, hasjoint,wall);
                }
            });
        }
    }

    function showWindowDoor(dataArray) {
        var model0 = null;
        var model1 = null;

        var loader = new PredefinedModelLoader();

        loader.load("models/predefined/door.json", function (object, animationOn, animationOff) {
            model0 = object;
            console.log(object, animationOn, animationOff);
            loader.load("models/predefined/window.json", function (object, animationOn, animationOff) {
                model1 = object;
                console.log(object, animationOn, animationOff);
                ...
                $.each(dataArray, function (idx, data) {
                    if (data.model.id == 0) {
                        var door = model0.clone();
                        var modelSize = new THREE.Box3().setFromObject(door).getSize();
                        var realSize = new THREE.Vector3(1.2,1.8,0.02) ;
                        var scale = realSize.clone ().multiplyScalar( V3DSettings.RESIZE_RATIO).divide(modelSize);
                        door.scale=scale;
                        door.rotateY( data.rotate/180 * Math.PI);
                        var h = data.height?data.height:0;
                        var position = new THREE.Vector3( data.position.x,h, data.position.y)
                            .multiplyScalar(V3DSettings.RESIZE_RATIO);
                        door.position.set(position.x,position.y,position.z);
                        floorGroup.add(door);
                        makeHole(door,{x:1.2,y:1.8,z:0.1})
                    }

                    else
                    {
                        var realSize = new THREE.Vector3(1.2,1,0.02) ;
                        var win = model1.clone();
                        var modelSize = new THREE.Box3().setFromObject(win).getSize();
                        var scale = realSize.clone ().multiplyScalar( V3DSettings.RESIZE_RATIO).divide(modelSize);
                        win.scale=scale;
                        win.rotateY( data.rotate/180 * Math.PI);
                        var position = new THREE.Vector3( data.position.x, data.height?data.height:0, data.position.y)
                                    .multiplyScalar(V3DSettings.RESIZE_RATIO);
                        win.position.set(position.x,position.y,position.z);
                        floorGroup.add(win);
                        makeHole(win,realSize)
                    }
    ...
    scope.loadFloorShape = function (data, floorId) {
        console.log('loadFloorShape');
        console.log(data);
        scope.floorShapedata = data;
        showShape(data.areas);
        showWall(data.walls);
        showWindowDoor(data.windowDoors);
        fitCameraToFloorSize(floorGroup);
    };


    scope.showAssetLabel = function () {
        $.each(placeNodes, function (idx, node) {
            node.showAssetLabel(true);
        })
    };
    ...
    scope.hideAssetLabel = function () {
        $.each(placeNodes, function (idx, node) {
            node.showAssetLabel(false);
        })
    };

    scope.showRackCapacity = function () {
        console.log('in rack capacity');
    ...
    scope.waterLeak = function (id, flag) {
        var pnode = getPnodeByDeviceId(id);
        pnode.showWaterLeak(flag);
    };


    scope.defineCableRoute = function (start) {
        if (start) {

            //flag == true start to define
            controls.enabled = false;
            var box = new THREE.Box3().setFromObject(floor);
            ...
            $.each(path, function (idx, item) {
                var cable = createCable(item, idx);
                scope.visualRoot.add(cable);
                cableDevices.push(cable.userData);
            });
            ...
            loadData(SERVICE.SaveCableDevices, postData);

            pa.clearData();
        }
    };

    function createCable(item, idx) {
        var f = new THREE.Vector3(item.from.x, item.from.y, item.from.z);
        var t = new THREE.Vector3(item.to.x, item.to.y, item.to.z);
        var d = f.distanceTo(t);
        var cableRadius = V3DSettings.WATER_CABLE_RADIUS;
        var geometry = new THREE.CylinderGeometry(cableRadius * V3DSettings.RESIZE_RATIO, cableRadius * V3DSettings.RESIZE_RATIO, d, 32, 1, true);
        geometry.rotateX(Math.PI / 2);
        geometry.rotateY(Math.PI / 2);
        var texture = new THREE.TextureLoader().load("images/cable_green.jpg");
        texture.wrapS = THREE.RepeatWrapping;
        texture.wrapT = THREE.RepeatWrapping;
        texture.repeat.set(d / 10, d / 10);
        var material = new THREE.MeshPhongMaterial({map: texture, side: THREE.DoubleSide});
        var cable = new THREE.Mesh(geometry, material);
        cable.position.set((f.x + t.x) / 2, 2 * cableRadius * V3DSettings.RESIZE_RATIO, (f.z + t.z) / 2);


        var v1 = new THREE.Vector2(f.x, f.z);
        var v2 = new THREE.Vector2(t.x, t.z);
        var v = v2.sub(v1);
        var angle = Math.atan2(v.y, v.x);
        cable.rotateY(-angle);
        ...
        var saveData = {
            Device: {
                name: "管线设备",
                width: d / V3DSettings.RESIZE_RATIO,
    ...
    scope.saveAutoWalk = function () {
        ...
        var saveData= {
            floor_id: usedRoomId,
            path: JSON.stringify(pa.getData())
        };
        loadData(SERVICE.FloorRouteSave, saveData, function (result) {
            console.log(result);
        })
    };
    ...
    scope.startAutoWalk = function () {
        ...
        */
        ...
        loadData(SERVICE.GetFloorRoute, param, function (result) {
            if(result){
                console.log(pa.getData());
                console.log(JSON.parse(result.path));
                aw.startWalk(JSON.parse(result.path));
            }
    ...
    scope.mountPlaceNode = function (device_id, location3d, rawData) {
        ...
        if (pnode.deviceData && (pnode.deviceData.category == 14 || pnode.deviceData.category == 15)) {
            makeHole(pnode);
        }
